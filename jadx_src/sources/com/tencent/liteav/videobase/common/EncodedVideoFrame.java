...
public class EncodedVideoFrame {
    ...
    public void updateNALTypeAccordingNALHeader() {
        ...
        while (true) {
            nextNALHeaderPos = getNextNALHeaderPos(nextNALHeaderPos, this.data);
            if (nextNALHeaderPos == -1 || nextNALHeaderPos >= this.data.remaining()) {
                return;
            }
            c cVar2 = c.UNKNOWN;
            c nalTypeFromH265NALHeader = isH265() ? getNalTypeFromH265NALHeader(this.data, nextNALHeaderPos) : getNalTypeFromH264NALHeader(this.data, nextNALHeaderPos);
            c cVar3 = this.nalType;
            if (cVar3 == null || cVar3 == cVar2 || nalTypeFromH265NALHeader == c.IDR) {

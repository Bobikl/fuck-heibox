...
public class PiiElider {
    ...
    static {
        ...
        DOMAIN_NAME = patternCompile2;
        LIKELY_EXCEPTION_LOG = Pattern.compile("\\sat\\sorg\\.chromium\\.[^ ]+.");
        WEB_URL = Pattern.compile("(?:\\b|^)((?:(http|https|Http|Https|rtsp|Rtsp):\\/\\/(?:(?:[a-zA-Z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)\\,\\;\\?\\&\\=]|(?:\\%[a-fA-F0-9]{2})){1,64}(?:\\:(?:[a-zA-Z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)\\,\\;\\?\\&\\=]|(?:\\%[a-fA-F0-9]{2})){1,25})?\\@)?)?(?:" + patternCompile2 + ")(?:\\:\\d{1,5})?)(\\/(?:(?:[a-zA-Z0-9 -\ud7ff豈-\ufdcfﷰ-\uffef\\;\\/\\?\\:\\@\\&\\=\\#\\~\\-\\.\\+\\!\\*\\'\\(\\)\\,\\_])|(?:\\%[a-fA-F0-9]{2}))*)?(?:\\b|$)");
    ...
    public static String elideUrl(String str) {
        ...
        while (matcher.find(i10)) {
            ...
            if (likelyToBeAppNamespace(strSubstring) || likelyToBeSystemNamespace(strSubstring)) {
                ...
                sb2.replace(iStart, iEnd, URL_ELISION);
                i10 = iStart + 24;
    ...
    public static String sanitizeStacktrace(String str) {
        ...
        for (int i10 = 1; i10 < strArrSplit.length; i10++) {
            if (strArrSplit[i10].startsWith("Caused by:")) {
                strArrSplit[i10] = elideUrl(strArrSplit[i10]);
            }
        }

...
public class WeiXinShareContent extends SimpleShareContent {
    ...
    public WXMediaMessage getWxMediaMessage(Context context, boolean z10, String str) {
        if (getmStyle() == 2 || getmStyle() == 3) {
            return z10 ? buildImageParamsWithFileProvider(context, str) : buildImageParams();
        }
        if (getmStyle() == 4) {
        ...
        if (getmStyle() == 64) {
            return z10 ? buildEmojiParamsWithFileProvider(context, str) : buildEmojiParams();
        }
        if (getmStyle() == 32) {
            return buildFileParams();
        }
        return getmStyle() == 128 ? buildMinApp() : buildTextParams();
    }

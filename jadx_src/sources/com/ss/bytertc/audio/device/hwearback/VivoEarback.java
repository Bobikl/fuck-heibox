...
public class VivoEarback implements IHardWareEarback {
    ...
    public int setEffect(int i10) {
        if (this.vivoKtvHelper != null) {
            printLog("setEffect value=" + i10);
            if (i10 >= 0 && i10 <= this.vivoKtvHelper.getEffectSize()) {
                this.vivoKtvHelper.setCustomMode(i10);
                return 0;
            }
            RXLogging.w(TAG, String.format("ByteAudioEarBackEffect vivo valid is [%d,%d], value is %d, ignore it", 0, Integer.valueOf(this.vivoKtvHelper.getEffectSize()), Integer.valueOf(i10)));
        }
        return -1;
    }
    ...
    public int setVolume(int i10) {
        ...
        printLog("setVolume volume=" + i10);
        this.vivoKtvHelper.setMicVolParam((int) ((((double) i10) * 15.0d) / 100.0d));
        return 0;
    }
}

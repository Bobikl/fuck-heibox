...
public enum RemoteAudioState {
    ...
    RemoteAudioState(int i10) {
        this.value = i10;
    }

    @CalledByNative
    public static RemoteAudioState fromId(int i10) {
        for (RemoteAudioState remoteAudioState : values()) {
            if (remoteAudioState.value() == i10) {
    ...
    public String toString() {
        int i10 = AnonymousClass1.$SwitchMap$com$ss$bytertc$engine$data$RemoteAudioState[ordinal()];
        if (i10 == 1) {
        ...
        if (i10 != 4) {
            return i10 != 5 ? "" : "kRemoteAudioStateFailed";
        }

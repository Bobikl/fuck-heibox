...
public enum ReturnStatus {
    ...
    RETURN_STATUS_AUDIO_DEVICE_CAN_NOT_USE(o.f97683k),
    RETURN_STATUS_AUDIO_DEVICE_INIT_FAILED(-108),
    RETURN_STATUS_AUDIO_DEVICE_START_FAILED(o.f97687m),
    RETURN_STATUS_NATIVE_IN_VALID(-201),
    RETURN_STATUS_VIDEO_TIMESTAMP_WARNING(-202);

    private int value;

    ReturnStatus(int i10) {
        this.value = i10;
    }

    @CalledByNative
    public static ReturnStatus fromId(int i10) {
        for (ReturnStatus returnStatus : values()) {
            if (returnStatus.value() == i10) {
                return returnStatus;
            }
        }
        return null;
